...
import { Eraser, AlertTriangle, Layout, Users, ShieldAlert, RotateCcw, ShieldX, UserMinus, Zap, RefreshCw, Trash2, ShieldOff, DatabaseBackup, History, CheckCircle2, Crown, Gift, ShoppingBag, Trophy, Globe, Smartphone, UserCog, Sparkles, ListRestart } from 'lucide-react';
...
const AdminMaintenance: React.FC<AdminMaintenanceProps> = ({ currentUser }) => {
  ...
  // 1. تصفير الكاريزما لجميع المستخدمين
  const handleResetAllCharms = async () => {
    if (!confirm('⚠️ تحذير: هل تريد تصفير كاريزما جميع المستخدمين في التطبيق؟')) return;
    if (!confirm('☢️ هذا الإجراء سيقوم بتصفير ترتيب كاريزما الغرف نهائياً.')) return;
  ...
  // 2. تصفير قائمة الملوك والداعمين (نقاط الثروة)
  const handleResetGlobalLeaderboard = async () => {
    if (!confirm('👑 هل تريد تصفير "قائمة الملوك والداعمين" بالكامل؟')) return;
    if (!confirm('☢️ سيتم مسح نقاط "الثروة" لجميع المستخدمين لتبدأ القائمة من الصفر. هل أنت متأكد؟')) return;

    setIsProcessing(true);
    setProcessStatus('جاري تصفير قائمة الملوك...');
    try {
      const usersSnap = await getDocs(collection(db, 'users'));
      const batch = writeBatch(db);
      let count = 0;
      usersSnap.forEach((userDoc) => {
        batch.update(userDoc.ref, { wealth: 0 });
        count++;
      });
      await batch.commit();
      alert(`✅ تم تصفير ثروة ${count} مستخدم. القائمة الآن فارغة تماماً!`);
    } catch (e) {
      alert('❌ فشلت عملية تصفير القائمة.');
    } finally {
      setIsProcessing(false);
      setProcessStatus('');
    }
  ...
  // 3. تصفير حقائب المقتنيات لجميع المستخدمين (الميزة الجديدة)
  const handleClearAllBags = async () => {
    if (!confirm('⚠️ تحذير: هل تريد حذف جميع المقتنيات من حقائب جميع المستخدمين؟')) return;
    if (!confirm('☢️ هذا الإجراء سيمسح كافة الإطارات والفقاعات والدخوليات التي اشتراها أو فاز بها الأعضاء. هل أنت متأكد؟')) return;

    setIsProcessing(true);
    setProcessStatus('جاري إفراغ الحقائب...');
    try {
      const usersSnap = await getDocs(collection(db, 'users'));
      const batch = writeBatch(db);
      let count = 0;
      usersSnap.forEach((userDoc) => {
        // تفريغ المقتنيات وإزالة الأغراض المرتداة حالياً
      ...
      await batch.commit();
      alert(`✅ تم إفراغ حقائب ${count} مستخدم بنجاح! عاد الجميع للمظهر الافتراضي.`);
    } catch (e) {
      alert('❌ فشلت عملية إفراغ الحقائب.');
    } finally {
      setIsProcessing(false);
      setProcessStatus('');
    }
  ...
  const handleResetMasterAccount = async () => {
    ...
    try {
      ...
      alert('✅ تم تصفير حسابك بنجاح! سيتم تحديث البيانات في جلستك الحالية فوراً.');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 text-right font-cairo" dir="rtl">
        ...
        <div className="bg-amber-500/10 border-2 border-amber-500/30 p-8 rounded-[3rem] shadow-2xl relative overflow-hidden">
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="text-center md:text-right">
              ...
              <p className="text-slate-400 text-sm font-bold mt-2">إعادة حسابك الحالي للصفر (كوينز، مستويات، مقتنيات) كأنك مستخدم جديد.</p>
            </div>
      ...
      {/* زر تصفير قائمة الملوك والداعمين */}
      <div className="bg-pink-600/10 border-2 border-pink-500/30 p-8 rounded-[3rem] shadow-2xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-center md:text-right">
            <h3 className="text-2xl font-black text-white flex items-center justify-center md:justify-start gap-3">
              <Trophy className="text-pink-500" size={32} /> تصفير قائمة الملوك والداعمين
            </h3>
            <p className="text-slate-400 text-sm font-bold mt-2">تصفير نقاط الثروة لجميع المستخدمين لمسح المتصدرين في القائمة العالمية.</p>
          </div>
          <button onClick={handleResetGlobalLeaderboard} disabled={isProcessing} className="px-10 py-5 bg-pink-600 hover:bg-pink-500 text-white font-black rounded-2xl shadow-xl transition-all active:scale-95 disabled:opacity-50 flex items-center gap-3">
            {isProcessing ? <RefreshCw className="animate-spin" /> : <ListRestart />} تصفير الملوك الآن
          </button>
        </div>
      </div>

      {/* زر تصفير حقائب المقتنيات لجميع المستخدمين - الميزة المطلوبة */}
      <div className="bg-purple-600/10 border-2 border-purple-500/30 p-8 rounded-[3rem] shadow-2xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-center md:text-right">
            <h3 className="text-2xl font-black text-white flex items-center justify-center md:justify-start gap-3">
              <ShoppingBag className="text-purple-500" size={32} /> تصفير الحقائب والمقتنيات
            </h3>
            <p className="text-slate-400 text-sm font-bold mt-2">حذف جميع الإطارات والفقاعات والدخوليات من جميع حسابات المستخدمين.</p>
          </div>
          <button onClick={handleClearAllBags} disabled={isProcessing} className="px-10 py-5 bg-purple-600 hover:bg-purple-500 text-white font-black rounded-2xl shadow-xl transition-all active:scale-95 disabled:opacity-50 flex items-center gap-3">
            {isProcessing ? <RefreshCw className="animate-spin" /> : <Trash2 />} تصفير الحقائب الآن
          </button>
        </div>
      </div>
      ...
      <div className="bg-indigo-600/10 border-2 border-indigo-500/30 p-8 rounded-[3rem] shadow-2xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-center md:text-right">
            <h3 className="text-2xl font-black text-white flex items-center justify-center md:justify-start gap-3">
              <RotateCcw className="text-indigo-400" size={32} /> تصفير كاريزما الجميع
            </h3>
            <p className="text-slate-400 text-sm font-bold mt-2">تصفير الكاريزما (نقاط استقبال الهدايا) لجميع المستخدمين في التطبيق.</p>
          </div>
          <button onClick={handleResetAllCharms} disabled={isProcessing} className="px-10 py-5 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-2xl shadow-xl transition-all active:scale-95 disabled:opacity-50 flex items-center gap-3">
            {isProcessing ? <RefreshCw className="animate-spin" /> : <Eraser />} تصفير الكاريزما للكل
          </button>
        </div>
      </div>
