...
import { Sparkles, Plus, Trash2, Edit3, Image as ImageIcon, Upload, Save, Calendar, Trophy, Coins, Crown, Layers, X, ShieldCheck, Target, Settings2, Video, MessageSquare, List, Activity as ActivityIcon, ShoppingBag, ChevronDown, Check, Layout, Monitor, Clock, Link as LinkIcon, Gift } from 'lucide-react';
          ...
          <motion.div key={act.id} className="bg-slate-900 border border-white/5 rounded-[2.5rem] overflow-hidden group hover:border-amber-500/30 transition-all shadow-2xl">
             ...
             <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-white font-black text-lg">{act.title}</h4>
                  <span className={`text-[8px] font-black px-2 py-0.5 rounded-full ${act.type === 'gift_event' ? 'bg-pink-500/20 text-pink-500' : 'bg-blue-500/20 text-blue-500'}`}>
                    {act.type === 'gift_event' ? 'تجميع كاريزما' : 'تجميع شحن'}
                  </span>
                </div>
                <div className="flex gap-2">
                   <button onClick={() => setEditingActivity(act)} className="flex-1 py-4 bg-blue-600/10 text-blue-500 rounded-2xl font-black text-[11px] hover:bg-blue-600 hover:text-white transition-all shadow-inner">تعديل</button>
          ...
          <div className="fixed inset-0 z-[3005] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-[#0f172a] border border-white/10 rounded-[3.5rem] w-full max-w-5xl p-10 shadow-2xl overflow-y-auto max-h-[92vh] custom-scrollbar">
              <div className="flex justify-between items-center mb-10">
                <h3 className="text-2xl font-black text-white flex items-center gap-3"><Settings2 className="text-amber-500" /> تخصيص الفعالية</h3>
              ...
              <div className="space-y-10">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-2">
                       <label className="text-[10px] font-black text-slate-500 uppercase pr-2">نوع التحدي</label>
                       <select 
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                       ...
                       {(!editingActivity.rewards || editingActivity.rewards.length === 0) && (
                          <div className="py-12 bg-black/20 rounded-[2.5rem] border border-white/5 border-dashed text-center opacity-30">
                             <List size={40} className="mx-auto mb-2" />
                             <p className="font-bold text-sm">لم تقم بإضافة أي جوائز لهذا النشاط بعد</p>
                          </div>
                       )}
                    </div>
                 </div>
                 ...
                 <button onClick={handleSaveActivity} className="w-full py-6 bg-gradient-to-r from-amber-600 to-orange-600 text-black font-black rounded-[2.5rem] shadow-xl active:scale-95 transition-all text-lg flex items-center justify-center gap-4">
                    <Save size={26} /> حفظ الفعالية المفعّلة
                 </button>
              </div>
            </motion.div>
          </div>
