...
const VoiceRoom: React.FC<any> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[150] flex flex-col bg-slate-950 font-cairo overflow-hidden text-right">
      ...
      <RoomHeader room={room} onLeave={onLeave} onMinimize={onMinimize} onShowParticipants={() => { onPushState?.(); setShowParticipants(true); }} />
      ...
      <AnimatePresence>{showProfileSheet && selectedUserForProfile && (<UserProfileSheet user={selectedUserForProfile} onClose={() => setShowProfileSheet(false)} isCurrentUser={selectedUserForProfile.id === currentUser.id} onAction={(action) => { if (action === 'gift') { onPushState?.(); setSelectedRecipientIds([selectedUserForProfile.id]); setShowGifts(true); setShowProfileSheet(false); } if (action === 'message') { onPushState?.(); onOpenPrivateChat(selectedUserForProfile); } if (action === 'edit') { onPushState?.(); setShowEditProfileModal(true); } if (action === 'cp') { onPushState?.(); onOpenCP(); } handleAdminActionFromProfile(action, selectedUserForProfile); }} currentUser={currentUser} allUsers={users} currentRoom={room} />)}</AnimatePresence>
