...
const AuthScreen: React.FC<AuthScreenProps> = ({ onAuth, appLogo, authBackground, canInstall, onInstall }) => {
  ...
  const [cachedIp, setCachedIp] = useState('0.0.0.0');
  ...
  useEffect(() => {
    ...
    // جلب الـ IP فوراً في الخلفية لتسريع الدخول لاحقاً
  ...
  const handleIdLogin = async () => {
    ...
    try {
      ...
      // محاولة البحث عن العضو بالآيدي المخصص أولاً (أسرع)
      ...
      if (!querySnapshot.empty) {
        ...
        // إذا لم يجد، يبحث بالآيدي الافتراضي
      ...
      if (targetUser && targetUser.loginPassword === password) {
        ...
        const devId = getDeviceId();
        // نستخدم الـ IP المخزن مسبقاً بدلاً من طلبه الآن
        setDoc(doc(db, 'users', targetUser.id), { deviceId: devId, lastIp: cachedIp }, { merge: true });
        onAuth({ ...targetUser, deviceId: devId, lastIp: cachedIp });
      } else {
    ...
    try {
      if (authMode === 'login') {
        ...
        if (userDoc.exists()) {
          ...
          // تحديث البيانات في الخلفية دون تعطيل الدخول
          setDoc(doc(db, 'users', uData.id), { deviceId: devId, lastIp: cachedIp }, { merge: true });
          onAuth({ ...uData, deviceId: devId, lastIp: cachedIp });
        }
      } else {
      ...
      <AnimatePresence mode="wait">
        {isActuallyBanned ? (
           ...
           >
              ...
              <p className="text-red-200/70 text-xs leading-relaxed font-bold">
